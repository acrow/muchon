...
(function(){if(document.body.classList.contains('home-template'))return;var h=document.getElementById('header');if(!h)return;var t=10;function u(){if(window.scrollY>t){h.classList.add('scrolled')}else{h.classList.remove('scrolled')}}window.addEventListener('scroll',u,{passive:true});u();})();
(function(){
  if(!window._bgSlideshow)return;
  var files=window._bgSlideshow.split(',').map(function(s){return s.trim();}).filter(Boolean);
  if(files.length<2)return;
  var assetsUrl=window._bgAssetsUrl||'';
  var interval=(window._bgInterval||6)*1000;
  var bg=document.querySelector('#wrapper > .bg');
  if(!bg)return;
  var idx=0;
  var overlayPart='url("'+assetsUrl+'/images/overlay.png"), linear-gradient(0deg,rgba(0,0,0,0.1),rgba(0,0,0,0.1)), ';
  function switchBg(){
    idx=(idx+1)%files.length;
    bg.style.opacity='0';
    setTimeout(function(){
      bg.style.backgroundImage=overlayPart+'url("'+assetsUrl+'/images/'+files[idx]+'")';
      bg.style.opacity='1';
    },900);
  }
